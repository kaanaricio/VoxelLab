...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _read_json_payload_or_413(self, max_bytes: int = MAX_API_JSON_BODY_BYTES) -> dict | None:
        try:
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/ask":
            ...
            if limited is not None:
                self._json(*limited)
                return
            body = self._read_json_payload_or_413()
            if body is None:
                return
        ...
        if parsed.path == "/api/cloud/get_upload_urls":
            payload = self._read_json_payload_or_413()
            if payload is None:
                return
