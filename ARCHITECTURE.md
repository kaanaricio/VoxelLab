...
graph TD
  ...
  state --> commands["js/state/viewer-commands.js"]
  state --> runtimeState["js/state/runtime-state.js"]
  ...
  upload --> dicomweb["js/dicomweb/dicomweb-source.js"]
  upload --> dicomsr["js/dicom-sr-export.js / js/dicom-sr-collect.js"]
  import --> parse["js/dicom-import-parse.js"]
  parse --> geometry["js/geometry.js"]
  parse --> worker["js/volume-worker-client.js"]
  dicomweb --> parse
  dicomweb --> transport["js/dicomweb/session-transport.js"]

  slice --> mpr["js/mpr-view.js"]
  slice --> compositor["js/slice-compositor.js"]
  mpr --> lanczos["js/lanczos.js"]
  mpr --> mprGeometry["js/mpr-geometry.js"]
  mpr --> mprProjection["js/mpr-projection.js"]
  mpr --> compositor
  mprGeometry --> geometry
  mprProjection --> geometry
  ...
  annotation["js/annotation.js"] --> annotationGraph["js/annotation-graph.js"]
  plugin --> state
  derived --> geometry
...
|---|---|---|
| `manifest`, `seriesIdx`, `sliceIdx` | `viewer.js` bootstraps, then `js/state/viewer-commands.js` via `js/select-series.js` and view controls | nearly all rendering modules |
| `mpr.*` | `js/state/viewer-commands.js`, called from `js/wire-controls-mpr-panel.js`, `js/sync.js`, and `js/mpr-view.js` | `js/mpr-view.js`, `js/slice-view.js`, `js/mpr-gpu.js` |
| `three.*` | `js/runtime/viewer-runtime.js`, `js/view-modes.js`, `js/volume-3d.js` | `js/volume-3d.js`, clip/readout UI, runtime readiness modules |
| `overlays.*` | `js/select-series.js`, `js/overlay-stack.js`, `js/wire-controls.js`, runtime overlay helpers | `js/slice-view.js`, `js/slice-compositor.js`, `js/mpr-view.js`, `js/volume-label-overlay.js` |
| `voxels`, `hrVoxels` | `js/volume-voxels-ensure.js`, `js/volume-hr-voxels.js`, cached through `js/runtime/viewer-runtime.js` | `js/slice-view.js`, `js/mpr-view.js`, `js/mpr-gpu.js`, `js/volume-3d.js` |
| local import caches (`_local*`) | `js/dicom-import.js`, `js/dicom-derived-import.js`, `js/dicomweb/dicomweb-source.js` import adapters | `js/select-series.js`, `js/volume-voxels-ensure.js`, `js/volume-hr-voxels.js` |
| measurements / annotations / ask | `js/measure.js`, `js/annotation.js`, `js/annotation-graph.js`, `js/consult-ask.js` | `js/slice-view.js`, side panels, DICOM SR export/report paths |
...
- [`js/dicomweb/session-transport.js`](js/dicomweb/session-transport.js): DICOMweb frame Accept negotiation, transfer-syntax verification, and session transport policy
- [`js/series-contract.js`](js/series-contract.js): normalized manifest/projection-set records shared by local imports and cloud results
...
- [`js/mpr-geometry.js`](js/mpr-geometry.js): orthogonal MPR plane sizing and voxel mapping
- [`js/mpr-projection.js`](js/mpr-projection.js): thin/MIP/average slab projection basis shared by CPU and GPU MPR
...
- [`js/annotation-graph.js`](js/annotation-graph.js) and [`js/dicom-sr-*.js`](js/dicom-sr-export.js): viewer annotation graph and DICOM SR import/export mapping
