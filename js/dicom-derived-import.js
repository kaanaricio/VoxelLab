...
function sliceIndexForIPP(series, ipp, toleranceMm = 1.2) {
  const [, , z] = voxelPointForLps(series, ipp);
...
export function buildSegOverlayImport(dataset, sourceSeries) {
  ...
  const frameByteCount = bitsAllocated === 1 ? Math.ceil(framePixelCount / 8) : framePixelCount;
  ...
  for (let index = 0; index < frames.length; index++) {
    ...
    const pixels = bitsAllocated === 1 ? bitPackedFrame(frameBytes, framePixelCount) : frameBytes.slice(0, framePixelCount);
  ...
  for (const contourGroup of meta.ROIContourSequence || []) {
    ...
    for (const contour of contourGroup?.ContourSequence || []) {
      ...
      for (let i = 0; i < raw.length; i += 3) {
        points.push(voxelPointForLps(sourceSeries, [raw[i], raw[i + 1], raw[i + 2]]));
      }
      const centroid = [
        raw.filter((_, index) => index % 3 === 0).reduce((sum, value) => sum + value, 0) / points.length,
        raw.filter((_, index) => index % 3 === 1).reduce((sum, value) => sum + value, 0) / points.length,
        raw.filter((_, index) => index % 3 === 2).reduce((sum, value) => sum + value, 0) / points.length,
      ];
