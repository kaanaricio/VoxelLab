...
const FRAME_ACCEPT = `application/octet-stream; transfer-syntax=${EXPLICIT_VR_LITTLE_ENDIAN}`;
...
function transferSyntaxFromContentType(contentType = '') {
  const match = String(contentType || '').match(/(?:^|;)\s*transfer-syntax\s*=\s*"?([^";\s]+)"?/i);
  return match ? match[1] : '';
}

function transferSyntaxFromAccept(accept = '') {
  return transferSyntaxFromContentType(accept);
}
...
class DicomwebTransportSession {
  ...
  async fetchFrameBytes({
    ...
    const pending = (async () => {
      ...
      const requestedTransferSyntax = transferSyntaxFromAccept(accept);
      const responseTransferSyntax = transferSyntaxFromContentType(contentType);
      if (requestedTransferSyntax && responseTransferSyntax && responseTransferSyntax !== requestedTransferSyntax) {
        throw new Error(`DICOMweb frame transfer syntax mismatch: requested ${requestedTransferSyntax}, got ${responseTransferSyntax}`);
      }
      const buffer = await response.arrayBuffer();
      const payload = normalizedContentType.includes('multipart/related')
        ? extractMultipartPart(buffer, contentType)
        : buffer;
      Object.defineProperty(payload, 'transferSyntaxUID', {
        value: responseTransferSyntax || requestedTransferSyntax || '',
        enumerable: false,
      });
      return payload;
    })();
