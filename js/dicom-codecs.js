...
async function decodeWithOpenJPEG(buffer, rows, cols, bitsAllocated) {
  ...
  try {
    ...
    const decoder = new _openjpeg.J2KDecoder();
    const encodedBuffer = decoder.getEncodedBuffer(encoded.length);
    encodedBuffer.set(encoded);
    decoder.decode();
    const decoded = decoder.getDecodedBuffer();
    if (bitsAllocated === 16) {
      return new Int16Array(decoded.buffer, decoded.byteOffset, rows * cols);
    }
    return new Uint8Array(decoded.buffer, decoded.byteOffset, rows * cols);
  } catch (e) {
...
async function decodeWithCharls(buffer, rows, cols, bitsAllocated) {
  ...
  try {
    const encoded = new Uint8Array(buffer);
    const decoder = new _charls.JpegLSDecoder();
    const encodedBuffer = decoder.getEncodedBuffer(encoded.length);
    encodedBuffer.set(encoded);
    decoder.decode();
    const decoded = decoder.getDecodedBuffer();
