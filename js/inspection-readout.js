...
export function resolveVoxelInspection(
  ...
  if (series.slug === 'dwi_adc' && state.stats?.adc && state.hrVoxels?.length === series.width * series.height * series.slices) {
    const adc = state.stats.adc;
    const norm = state.hrVoxels[vi];
    const rawDicom = adc.hr_lo_raw + norm * (adc.hr_hi_raw - adc.hr_lo_raw);
    const physical = rawDicom * adc.rescale_slope + adc.rescale_intercept;
    adcDisplay = physical / adc.display_divisor;
  }
  return {
